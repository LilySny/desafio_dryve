...
class _VehicleCardState extends State<VehicleCard> {
  ...
  void _addOrRemoveFromFavorites() {
    ...
    if (!_favoriteStore.vehicles.contains(widget.model.id)) {
      _favoriteStore.add(widget.model.id);
      _showSnackBar(
          "${model.brandName} ${model.modelName} Adicionado aos favoritos",
          Icons.check_circle,
          const Color.fromRGBO(0, 101, 255, 1));
    } else {
      _favoriteStore.remove(widget.model.id);
      _showSnackBar(
          "${model.brandName} ${model.modelName} Removido dos favoritos",
